...
public class AnalyzeOpenPGPFile {
	...
	public static void main(String[] args) throws IOException, PGPException {
		...
		pgp.encryptFile(SRC + "examples/DataFiles/INPUT.txt", SRC + "examples/DataFiles/public.key", openpgpFile, asciiArmor);
		...
		if (inspect.isSignedOnly(openpgpFile)) {
			...
			String privateKeyFileName = SRC + "examples/DataFiles/private.key";
